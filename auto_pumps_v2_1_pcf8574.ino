...
void setup() {
  ...
  for (byte i = 0; i <= PUPM_AMOUNT; i++) {        // пробегаем по всем помпам
    ...
    if (SWITCH_LEVEL)                              // вырубить все помпы
      ...
      pump_state[i] = 1;
    lcd.setCursor(i, 3);  // заполняем индикатор заданий звездами
    lcd.print("*");
  }
...
void flowTick() {                                               // выключение зоны
  for (byte i = 0; i < PUPM_AMOUNT; i++) {                      // пробегаем по всем помпам
    if (pumping_time[i] > 0                                     // если время полива больше нуля
      ...
      lcd.print("PAUSE     ");
      lcd.setCursor(i, 3);  // помечаем зону как политую на индикаторе зон
      lcd.print("*");
...
void encoderTick() {
  ...
  if (enc1.isHolded()) {                            // ----удержание кнопки
    ...
    lcd.setCursor(current_pump, 3);                 // помечаем зону как не политую на индикаторе зон
    lcd.print("0");
  }
